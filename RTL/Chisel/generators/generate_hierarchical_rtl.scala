// Run from RTL/Chisel sbt 'runMain generators.GenerateHierarchicalRTL'
...
object GenerateHierarchicalRTL extends App {
  ...
  def generateFIRRTLandSystemVerilog(chiselModule: () => chisel3.Module, moduleName: String): Unit = {
    println(s"${moduleName}: generateFIRTLandSystemVerilog")
  ...
  def generateHierarchicalSystemVerilog(chiselModule: () => chisel3.Module, moduleName: String): Unit = {
    ...
    if(isCmdInstalled("firtool")) {      
      ...
      println(s"$moduleName - firtool command = $firrtlCommand")

      val result = firrtlCommand.!  // Execute the command
