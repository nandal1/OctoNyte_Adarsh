...
#define OUTPUT_FILE "blt_dump.txt"
...
// Function to be used with `blt`
void compare_values() {
    __asm__ volatile (
        "li t0, 5\n"          // Load 5 into t0
        "li t1, 10\n"         // Load 10 into t1
        "blt t0, t1, less_than\n"  // If t0 < t1, jump to less_than
        "li a0, 0\n"          // If t0 >= t1, set a0 = 0
        "j end\n"             // Jump to end
...
int main() {
    ...
    // Call the function that executes `blt`
    __asm__ volatile (
        "call compare_values\n"
    );

    // Capture register state after calling `blt`
    get_registers(&state);
