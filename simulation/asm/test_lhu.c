...
int main() {
    ...
    /* LHU Example */
    uint16_t memory_value = 0x1234;  // Halfword (16-bit) value in memory
    uint16_t loaded_value = 0;       // Variable to store loaded value

    __asm__ volatile (
        "mv t0, %1\n"   // Move address of memory_value into t0
        "lhu t1, 0(t0)\n" // Load halfword unsigned from memory into t1
        "mv %0, t1\n"    // Move loaded value into loaded_value
        : "=r"(loaded_value)  // Output operand
        : "r"(&memory_value)  // Input operand (memory address)
        : "t0", "t1"  // Clobbered registers
    );

    printf("LHU Execution Result: loaded_value = 0x%04X (%d)\n", loaded_value, loaded_value);

    get_registers(&state);
    regs.states[regs.count++] = state;

    // Save register dumps
    save_register_dump(&regs, "../test_compare/test_lhu_dump.txt");
