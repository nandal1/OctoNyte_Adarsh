...
int main() {
    ...
    // Test SB (Store Byte)
    unsigned char memory_array[1024];  // Array to store byte in memory
    unsigned char value_to_store = 0x55;  // Value to store (byte)
    unsigned long address_to_store = (unsigned long)&memory_array[100];  // Address to store at
    ...
    printf("SB Execution Result: Stored value 0x%02X at address 0x%lx\n", value_to_store, address_to_store);

    // Get register state after executing SB
    get_registers(&state);
    print_register_state(&state);
    regs.states[regs.count++] = state;
