...
void get_registers(RegisterState *regs) {
    ...
    __asm__ volatile (
        ...
        : "=r"(regs->a0), "=r"(regs->a1), "=r"(regs->a2), "=r"(regs->a3),
          "=r"(regs->a4), "=r"(regs->a5), "=r"(regs->a6), "=r"(regs->a7)
    );
    ...
    __asm__ volatile (
        ...
        "mv %8, s10\n"
        "mv %9, s11\n"
        : "=r"(regs->s2), "=r"(regs->s3), "=r"(regs->s4), "=r"(regs->s5),
          "=r"(regs->s6), "=r"(regs->s7), "=r"(regs->s8), "=r"(regs->s9),
          "=r"(regs->s10), "=r"(regs->s11)
    );
    ...
    __asm__ volatile (
        ...
        "mv %2, t5\n"
        "mv %3, t6\n"
        : "=r"(regs->t3), "=r"(regs->t4), "=r"(regs->t5), "=r"(regs->t6)
    );
}
...
int main() {
    ...
    // Perform SLT operation: t2 = (t0 < t1) ? 1 : 0
    __asm__ volatile (
        "slt t2, t0, t1\n"
    );

    // Capture state after SLT
    get_registers(&state);
    save_register_dump(&state);
