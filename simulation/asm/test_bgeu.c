...
#define OUTPUT_FILE "bgeu_dump.txt"
...
// Function to be used with `bgeu`
void compare_unsigned_values() {
    __asm__ volatile (
        "li t0, 20\n"          // Load unsigned 20 into t0
        "li t1, 15\n"          // Load unsigned 15 into t1
        "bgeu t0, t1, greater_equal_unsigned\n"  // If t0 >= t1 (unsigned), jump
...
int main() {
    ...
    // Call the function that executes `bgeu`
    __asm__ volatile (
        "call compare_unsigned_values\n"
    );

    // Capture register state after calling `bgeu`
    get_registers(&state);
