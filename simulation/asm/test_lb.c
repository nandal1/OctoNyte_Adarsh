...
int main() {
    ...
    uint8_t memory[4] = {0x12, 0x34, 0x56, 0x78};  // Example memory values
    unsigned long base_address = (unsigned long)&memory[0];  // Base address (rs1)
    unsigned long offset = 0x008;  // Offset (immediate value)
    unsigned long address = base_address + offset;  // Effective address

    // Load byte from memory into t1 using LB instruction
    __asm__ volatile (
        "lb %0, 0(%1)"  // Load byte at address into t1
        : "=r"(state.t1)  // Output operand (t1)
        : "r"(address)    // Input operand (address)
    );

    printf("Initial register state:\n");
    printf("  t1 (loaded byte) = 0x%lx\n", state.t1);

    // Save the register dump after LB operation
    regs.states[regs.count++] = state;
    save_register_dump(&regs, "../test_compare/test_lb_dump.txt");
