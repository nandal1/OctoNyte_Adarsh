...
// Function used in JALR example
int test_function() {
    return 0xDEADBEEF;  // Just a marker return value
}
...
int main() {
    ...
    /* JALR Example */
    __asm__ volatile (
        "auipc t0, 0\n"         // Load current PC into t0 (Upper Immediate)
        "addi  t0, t0, 12\n"    // Adjust t0 to hold address of test_function
        "jalr  ra, t0, 0\n"     // Jump and link to test_function
    );
    ...
    // Save register dumps
    save_register_dump(&regs, "../test_compare/test_jalr_dump.txt");
