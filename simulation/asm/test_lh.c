...
int main() {
    ...
    int16_t memory_value = 0x1234;  // Halfword (16-bit) value in memory
int16_t loaded_value = 0;       // Variable to store loaded value

__asm__ volatile (
    "mv t0, %1\n"   // Move address of memory_value into t0
    "lh t1, 0(t0)\n" // Load halfword from memory into t1
    ...
    printf("LH Execution Result: loaded_value = 0x%04X (%d)\n", loaded_value, loaded_value);

    get_registers(&state);
    regs.states[regs.count++] = state;

    // Save register dumps
    save_register_dump(&regs, "../test_compare/test_lh_dump.txt");
