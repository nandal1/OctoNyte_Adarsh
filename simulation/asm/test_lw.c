...
int main() {
    ...
    /* LW Example */
    int32_t memory_value = 0x12345678;  // 32-bit (word) value in memory
    int32_t loaded_value = 0;           // Variable to store loaded value

    __asm__ volatile (
        "mv t0, %1\n"   // Move address of memory_value into t0
        "lw t1, 0(t0)\n" // Load word from memory into t1
    ...
    printf("LW Execution Result: loaded_value = 0x%08X (%d)\n", loaded_value, loaded_value);

    get_registers(&state);
    regs.states[regs.count++] = state;

    // Save register dumps
    save_register_dump(&regs, "../test_compare/test_lw_dump.txt");
