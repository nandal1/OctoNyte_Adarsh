...
int main() {
    ...
    /* LBU Example */
    uint8_t memory_value = 0xAB;  // Byte value in memory (unsigned byte)
    uint8_t loaded_value = 0;     // Variable to store loaded value

    __asm__ volatile (
        "mv t0, %1\n"   // Move address of memory_value into t0
        "lbu t1, 0(t0)\n" // Load byte unsigned from memory into t1
    ...
    printf("LBU Execution Result: loaded_value = 0x%02X (%d)\n", loaded_value, loaded_value);

    get_registers(&state);
    regs.states[regs.count++] = state;

    // Save register dumps
    save_register_dump(&regs, "../test_compare/test_lbu_dump.txt");
